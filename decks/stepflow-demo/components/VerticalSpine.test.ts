 ...
 */
function mountSpine(props: { nodes: SpineNode[]; title?: string; titleAccent?: string }) {
...
describe('VerticalSpine', () => {
  ...
  it('slots the side cards at the measured chainBlue positions with white titles and captions', () => {
    const wrapper = mountSpine({ nodes: spineNodes })
    ...
    expect(cards[0].attributes('fill')).toBe('#349aea')
